...
from ActFormer.RoutesFormer.toyact_src.actformer_toyact import Routesformer
...
trip_df = pd.read_csv(os.path.join(base_path, 'toy_data_generation/route_data/route.csv'), index_col = 0) # 仮想経路データ
trip_df = trip_df.iloc[:, :-1]
trip_arr = trip_df.to_numpy()
#シミュレーション用のデータセット
...
teacher_df.to_csv(os.path.join(base_path, 'toy_data_generation/route_data/teacher.csv'))
...
feature_dim = network.node_features.shape[1] #+ 1 #なぜか+1されてるが，，，
...
#RoutesFormerのハイパーパラメータ
l_max = 14+2 # wandb.config.l_max #シークエンスの最大長さ
B_en = 6 # wandb.config.B_en #エンコーダのブロック数 # 元論文より
B_de = 6 # wandb.config.B_de #デコーダのブロック数 # 元論文より
head_num = 4 # wandb.config.head_num #ヘッド数
d_ie = 22 # wandb.config.d_ie #トークンの埋め込み次元数
d_fe = 2 # wandb.config.d_fe #特徴の埋め込み次元数 d_ie+d_feがhead_numで割り切れるように設定
d_ff = 32 # wandb.config.d_ff #フィードフォワード次元数
# eos_weight = 3.0 # wandb.config.eos_weight #EOSトークンの重み
# savefilename = "toydata.pth" # wandb.config.savefilename #モデルの保存ファイル名
...
model = Routesformer(enc_vocab_size= vocab_size,
                            ...
                            B_de = B_de).to(device)
# print('modelは回ってる')
model_weights_path = "/Users/matsunagatakahiro/Desktop/res2025/ActFormer/RoutesFormer/output/toydata.pth" # 学習済みmodelのアウトプット
loadfile = torch.load(model_weights_path)
print('loadfile ha dekiteru')
model.load_state_dict(loadfile['model_state_dict']) ## ここでエラー
print('model ha mawatteru')
model.eval()
print('kokomadekiteru')
tokenizer = Tokenization(network)
ignore_value_list = [tokenizer.SPECIAL_TOKENS["<p>"], tokenizer.SPECIAL_TOKENS["<m>"]]
neighbor = Neighbor(adj_matrix)
print('kokomadekiteru2')
########################################
# 4. 推論用の関数を分割して定義
########################################
...
def check_and_save_completed_routes(
    traveled_route, traveled_feats, l_max, tokenizer, idx_lis,
     #  time_is_day, 
    d_tensor,
...
def run_inference(test_loader, model, tokenizer, neighbor, 
                #  img_dic,
                l_max=62):
    """
    ...
    for batch_idx, (disc_tokens) in enumerate(test_loader):
        ...
        while i <= l_max - 1:
            ...
            traveled_route, traveled_feats = update_traveled_route(
                tokenizer, traveled_route, next_zone
...
result = run_inference(test_loader, model, tokenizer, neighbor,
                        # img_dic, 
                        l_max=l_max)
