...
class ContextEmbedding(nn.Module):
    def __init__(self, context_vocab_sizes, d_model): # context_vocab_sizesは
        ...
        # self.age_emb = nn.Embedding(context_vocab_sizes[0], d_model) 
        # self.gender_emb = nn.Embedding(context_vocab_sizes[1], d_model)
    ...
    def forward(self, context_tokens):  # context_tokens.shape = [batch_size, 4]
        ...
        # 全てのベクトルを連結して最終的なコンテキストベクトルを作る
        ctx_emb = torch.concat([gender_vec, age_vec], dim = -1)
...
class MultiHeadAttention(nn.Module):
  ...
  def forward(self, encodings_for_q, encodings_for_k, encodings_for_v, attn_mask, padd_mask): # maskが与えられればMask部分を追加するだけで基本的に同じ
    ...
    # print('QK.shape', QK.shape, 'attn_mask.shape, padd_mask.shape)', attn_mask.shape, padd_mask.shape)
    # print('QK', QK, 'atten_mask', attn_mask, 'padd_mask', padd_mask)
...
class TriModalBlock(nn.Module):
    def __init__(self, d_model, nhead, d_ff, dropout=0.1,
                 tri_modal_idx=True, # cross-modal index
                 ):
        super().__init__()
        # 自己注意
        self.sa = nn.ModuleList([MultiHeadAttention(d_model, nhead) for _ in range(2)])
        # 交差注意
        self.ca = nn.ModuleList([MultiHeadAttention(d_model, nhead) for _ in range(2)])
        self.ff = FeedForward(d_model, d_ff, dropout)
        self.norm1 = nn.ModuleList([nn.LayerNorm(d_model) for _ in range(2)])
        self.norm2 = nn.ModuleList([nn.LayerNorm(d_model) for _ in range(2)])
        self.dropout = nn.Dropout(dropout)
        self.tri_modal_idx = tri_modal_idx # cross-modal index
...
class DecoderforGPT(nn.Module):
  ...
  def __init__(self, # dec_vocab_size, # token_emb_dim, 
               ...
               use_tri_modal = False #use_tri_modal
               ):
  ...
  def forward(self,
    ...
    for blk in self.blocks:
      ...
      time_emb, act_emb = blk(time_emb, act_emb, attn_mask, pad_mask_seq)
